...
if (isset($restrict_url) && substr($src_host, strlen($src_host)-strlen($restrict_url)) != $restrict_url) {
	...
	$str= "document.write('<div class=\"col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1\">');\n";
	...
	if  (!$rss) {
		...
		// begin item listing
		$str.= "document.write('<div class=\"post-preview\">');\n";
		...
		foreach ( $all_items as $item ) {
			...
			if ($desc < 0) {
				...
				$str.= "document.write('<a href=\"" . trim($my_url) . "\"" . $target_window . '><h2 class="post-title">' . $my_title . '</h2></a><p class="post-meta">Posted by ' .  $author_str . " on ".$pretty_date."</p>');\n";
